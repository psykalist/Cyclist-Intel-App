...
- **New one-click workflow + "never lose a photo" guard.** Added `.github/workflows/fix-photos.yml` ("Refresh Rider Photos (manual)", self-hosted, `workflow_dispatch`) that runs `fetch_missing_photos.py` and commits `rider_photos.json`/`data.json` — so refreshing photos is a one-click Actions run like the teams/profiles workflows, no local script needed. Two layers of shrink protection: `fetch_missing_photos.py` now records the renderable-photo count up front and **aborts the write** if it would drop (it only ever adds/replaces, never removes), and the workflow's commit step **refuses to push** a `rider_photos.json` with fewer entries than origin already has.
- **Applying the fixes needs a manual run — a push alone won't show them.** The twice-daily cron is `--results-only` (no calendar discovery, no photos). After pushing: run the `update-data.yml` ("Full Scrape - Calendar & Teams") workflow to pick up Portugal, and run the new `fix-photos.yml` ("Refresh Rider Photos") workflow to rewrite the 332 photos.
